...
- [ ] `src/domain/mod.rs` に `pub mod render;` を追加
- [ ] `src/domain/render.rs` にトレイトを定義
...
- [ ] `src/infra/mod.rs` を作成（`pub mod chrome_renderer;`）
- [ ] `src/infra/chrome_renderer.rs` に以下を実装
...
- [ ] `.github/workflows/release.yml` を作成（kalidokit-rust の release.yml を参考）
...
- [ ] `scripts/setup.sh` を作成（kalidokit-rust の setup.sh を参考に drawio-tools 向けに書き換え）
...
- [ ] 上記リストを1項目ずつ実行し、失敗した項目があれば原因を調査して修正する
- [ ] 全項目がパスするまで繰り返し修正する
