...
#[derive(Parser)]
#[command(name = "drawio-cmd", version, about = "Draw.io PNG/XML bidirectional conversion CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Parser)]
#[command(name = "drawio-cmd", version, about)]
pub struct Cli {
    #[command(subcommand)]
...
fn test_extract_outputs_xml_to_stdout() {
    Command::cargo_bin("drawio-cmd").unwrap()
        .arg("extract")
...
- [ ] Docker ビルド確認 (skipped: no Docker in environment)
  - `docker build -t drawio-cmd:phase1 .` が成功すること
  - `docker run --rm drawio-cmd:phase1 --help` でヘルプが表示されること
  - `docker run --rm -v $(pwd)/tests/fixtures:/data drawio-cmd:phase1 extract /data/simple_embedded.png` で XML が出力されること
...
- [ ] ビルド確認
  ...
  - `cargo fmt --check` がフォーマット済みであること
- [ ] Docker ビルド確認
  - `docker build -t drawio-cmd:phase2 .` が成功すること
  - `docker run --rm drawio-cmd:phase2 --help` で extract / embed / export が表示されること
  - エラーが発生する場合はエラーが出なくなるまで修正を繰り返す
...
- [x] `scripts/setup.sh` を作成 ✅ 2026-03-19（kalidokit-rust の setup.sh を参考に drawio-cmd 向けに書き換え）

```bash
#!/bin/sh
set -e
DEFAULT_INSTALL_PATH="/usr/local/bin"
REPO="tk-aria/drawio-cmd"  # ← 実際のリポジトリに変更
BINARY_NAME="drawio-cmd"
...
主な変更点:
- `REPO`, `BINARY_NAME` を drawio-cmd 向けに変更
...
```bash
docker build -t ghcr.io/granizm/drawio-cmd:latest .
docker run --rm ghcr.io/granizm/drawio-cmd:latest --help
docker run --rm -v $(pwd)/tests/fixtures:/data ghcr.io/granizm/drawio-cmd:latest extract /data/simple_embedded.png
```
...
docker push ghcr.io/granizm/drawio-cmd:latest
docker tag ghcr.io/granizm/drawio-cmd:latest ghcr.io/granizm/drawio-cmd:v0.1.0
docker push ghcr.io/granizm/drawio-cmd:v0.1.0
```
...
- [ ] ビルド確認
  ...
  - `docker build -t drawio-cmd:phase3 .` が成功すること
  - `docker run --rm drawio-cmd:phase3 --help` が正常表示すること
  - `docker run --rm -v $(pwd)/tests/fixtures:/data drawio-cmd:phase3 extract /data/simple_embedded.png` が XML を出力すること
  - エラーが発生する場合はエラーが出なくなるまで修正を繰り返す
- [ ] リリースワークフロー検証
...
### extract コマンド
- [ ] `drawio-cmd extract tests/fixtures/simple_embedded.png` → stdout に XML 出力
- [ ] `drawio-cmd extract tests/fixtures/simple_embedded.png -o out.drawio` → ファイル出力
- [ ] `drawio-cmd extract tests/fixtures/plain.png` → エラーメッセージ表示
- [ ] `drawio-cmd extract nonexistent.png` → エラーメッセージ表示
- [ ] `echo "" | drawio-cmd extract /dev/stdin` → エラーメッセージ表示

### embed コマンド
- [ ] `drawio-cmd embed tests/fixtures/simple.drawio tests/fixtures/plain.png -o out.png` → 埋め込み PNG 生成
- [ ] 生成した out.png を extract で再抽出 → 元の XML と一致確認
- [ ] 存在しない XML ファイル指定 → エラー
- [ ] 存在しない PNG ファイル指定 → エラー

### export コマンド (render feature 有効時)
- [ ] `drawio-cmd export tests/fixtures/simple.drawio -o out.png` → PNG 生成 + XML 埋め込み
- [ ] `drawio-cmd export tests/fixtures/simple.drawio -o out.png --scale 2.0` → スケール変更
- [ ] 生成 PNG を extract で XML 抽出可能か確認

### CLI 一般
- [ ] `drawio-cmd --help` → ヘルプ表示
- [ ] `drawio-cmd --version` → バージョン表示
- [ ] `drawio-cmd extract --help` → サブコマンドヘルプ表示

### Docker
- [ ] `docker run --rm ghcr.io/granizm/drawio-cmd:latest --help`
- [ ] `docker run --rm -v $(pwd)/tests:/data ghcr.io/granizm/drawio-cmd:latest extract /data/fixtures/simple_embedded.png`

### setup.sh
- [ ] `sh scripts/setup.sh install` → バイナリインストール
- [ ] `drawio-cmd --version` → インストール確認
...
```bash
docker build -t ghcr.io/granizm/drawio-cmd:latest .
docker build -t ghcr.io/granizm/drawio-cmd:v0.1.0 .
docker push ghcr.io/granizm/drawio-cmd:latest
docker push ghcr.io/granizm/drawio-cmd:v0.1.0
```
...
```bash
docker pull ghcr.io/granizm/drawio-cmd:latest
docker run --rm ghcr.io/granizm/drawio-cmd:latest --help
```
...
```bash
git remote add origin https://github.com/tk-aria/drawio-cmd.git  # ← 実際のリポジトリ
...
- [ ] GitHub Actions で Release ジョブが完了し、以下のアーティファクトが添付されていることを確認:
  - `drawio-cmd-v0.1.0-x86_64-unknown-linux-musl.tar.gz`
  - `drawio-cmd-v0.1.0-aarch64-apple-darwin.tar.gz`
  - `drawio-cmd-v0.1.0-x86_64-pc-windows-msvc.zip`
...
which drawio-cmd  # → not found であること
```
